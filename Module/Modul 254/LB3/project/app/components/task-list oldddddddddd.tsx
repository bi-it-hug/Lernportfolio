...
export function TaskList({ data }: { data: Task[] }) {
    ...
    return (
        <TaskListBox>
            <TaskListHeader />
            <ul className="flex flex-col w-full h-fit bg-neutral-900 rounded-lg">
                {tasks.map((task) => (
...
export function TaskListHeader() {
    return (
        <div className="flex justify-start items-center py-1.75 gap-2 bg-neutral-900 text-sm font-normal w-full whitespace-nowrap rounded-lg">
...
export function TaskName({
    ...
    return (
        <div className="relative text-sm font-normal w-fit whitespace-nowrap">
            <p
                className={`${task.completed ? "dark:text-neutral-50/50 italic after:w-full after:opacity-100" : "dark:text-neutral-50/80"} ${isEditing ? "opacity-0" : ""} leading-none mb-0.5 after:absolute after:content-[] after:inset-y-0 after:left-0 after:w-0 after:opacity-0 after:h-0.5 after:m-auto dark:after:bg-neutral-50/80`}
            >
                {children}
            </p>
            <input
...
export function Bulletpoint({ task }: { task: Task }) {
    return <ChevronRight className={`icon ml-2.5 ${task.completed ? "dark:text-neutral-50/50" : "dark:text-neutral-50/80"}`} />;
}
