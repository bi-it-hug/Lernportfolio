...
def process_entries(import_data, uncommon_chars, author, password_piece_length, password_pieces_amount):
    ...
    for entry in import_data:
        ...
        if name_key not in name_count:
            name_count[name_key] = 1
            
        else:
            name_count[name_key] += 1
        
        email = entry['email'] = create_email(first_name, last_name, uncommon_chars, name_count[name_key])
...
def insert_custom_data(import_location):
    ...
    data = {
        ...
        'last_name': 'Moffet',
        'gender': 'Male',
        'street': 'Kurzschwingerweg',
        'street_number': '3274',
        'zip_code': '283829249',
        'city': 'Birkhausen'
    }
    
    with open(import_location, mode='a', encoding='utf-8', newline='') as file:
...
def main():
    ...
    insert_custom_data(import_location)
